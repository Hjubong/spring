...
public class TestController {
	...
	public String list(Model model, String type, String word, String word2, @RequestParam(defaultValue = "0") int slop) {
		...
		try {
			...
			HttpHost host = new HttpHost("172.25.54.62", 9200, "http");
...
//			접근하려는 인덱스 참조 > spring 인덱스 
			...
			*/
			...
			if (type.equals("match_all")) {
				...
				//match > 풀텍스트로 검색
				searchSourceBuilder.query(QueryBuilders.matchQuery("message", word)); //"컬럼명", 검색어
	...
	public String addok(Model model, String id, String message) {
		...
		try {
			...
			//client 객체
			RestHighLevelClient client = new RestHighLevelClient(RestClient.builder(new HttpHost("172.25.54.62", 9200, "http")));
